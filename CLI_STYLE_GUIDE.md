...
- 既存ファイルが検出された場合
  ```
  [WARN]  既存ファイルを検出: ~/.git-completion.bash
  [WARN]  .vimrc が既に存在します: ~/.vimrc
  ```
- 処理がスキップされた場合
  ```
...
| 関数 | プレフィックス | 色 | 用途 |
|------|---------------|------|------|
| `info` | `[INFO]` | 青 | 情報表示（配置先パス、次の操作案内など） |
| `success` | `[OK]` | 緑 | 処理の成功 |
| `warn` | `[WARN]` | 黄 | 警告（既存ファイル検出、スキップなど） |
| `error` | `[ERROR]` | 赤 | エラー（処理失敗、必須コマンド不足など） |
